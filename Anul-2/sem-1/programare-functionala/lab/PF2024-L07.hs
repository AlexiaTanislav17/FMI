...
--expToArb :: Expr -> Tree
--expToArb (Const t) = Lf t1
--expToArb (e1 :+: e1) = Node Add (expToArb e1) (expToArb e2)
--expToArb (e1 :*: e1) = Node Mult (expToArb e1) (expToArb e2)
...
data IntSearchTree value
  ...
  | BNode
      ...
      (Maybe value)             -- valoarea elementului...maybe e optional adica poate sa fie nimic
...
lookup' key (BNode left treeKey treeValue right)
  ...
  | treeKey > key = lookup' left

-- apelam asa:   lookup' 2 (BNode 3 4)  cred
