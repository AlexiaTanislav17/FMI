...
-- arbore vid
-- arbore cu valoare de tip Int in radacina si 3 fii    
...
-- cu extree testezi. de ex in ghci scrii level extree si ar trebui sa iti dea 3
...
instance (Scalar a) => Vector Vec3D a where
  ...
  smult k (Vec3D x1 y1 z1) = Vec3D (mult k x1) (mult k y1) (mult k z1)
  negatev (Vec3D x1 y1 z1) = Vec3D (negates x1) (negates y1) (negates z1)
