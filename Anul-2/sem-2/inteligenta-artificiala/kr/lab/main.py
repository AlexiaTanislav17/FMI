...
def min_max(stare):
    #daca am ajuns la adancimea maxima sau jocul a ajuns intr-o stare finala, returnez estimarea
    if stare.tabla_joc.finla() or stare.adancime==0:
        stare.estimare = stare.tabla_joc.estimeaza_scor(stare.adancime)
        return stare
    #caut mutari posibile
    mutariCulEstimare = [min_max(mutare) for mutare in stare.mutari()]
    if stare.j_curent == InfoJoc.JMAX:
        stare.stare_aleasa = max(mutariCulEstimare, key=lambda x: x.estimare)
    else: 
        stare.stare_aleasa = min(mutariCulEstimare, key=lambda x: x.estimare)
...
def alpha_beta(alpha, beta, stare):
    ...
    if stare.j_curent == InfoJoc.JMAX:
        ...
        for mutare in stare in stare.mutari():
            mutareCulEstimare = alpha_beta(alpha, beta, mutare)
            if stare.estimare < mutareCulEstimare.estimare:
                stare.estimare = mutareCulEstimare.estimare
                stare.stare_aleasa = mutareCulEstimare
                if alpha < mutareCulEstimare.estimare:
                    alpha = mutareCulEstimare.estimare
                    if alpha >= beta:
                        return stare
    else:
        stare.estimare = float('inf')
        for mutare in stare in stare.mutari():
            ...
            if stare.estimare > mutareCulEstimare.estimare:
                stare.estimare = mutareCulEstimare.estimare
                stare.stare_aleasa = mutareCulEstimare
                if beta > mutareCulEstimare.estimare:
                    beta = mutareCulEstimare.estimare
                    if alpha >= beta:
                        return stare
